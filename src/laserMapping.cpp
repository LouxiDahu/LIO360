...
void publish_odometry(const ros::Publisher & pubOdomAftMapped)
{
    ...
    br.sendTransform( tf::StampedTransform( transform, odomAftMapped.header.stamp, "camera_init", "livox_frame" ) );
}
    ...
    while (status)
    {
        ...
        if(sync_packages(Measures)) 
        {
            ...
            if(!init_map)
            {
                //如果使用PCD先验
                if(Pcd_pre){
                    pcl::PointCloud<pcl::PointXYZINormal>::Ptr cloud_from_pcd(new pcl::PointCloud<pcl::PointXYZINormal>);

                    if (pcl::io::loadPCDFile<pcl::PointXYZINormal>("/home/rm/rm_sentry_ws/src/ws_livox/src/Point-LIO/PCD_Saver/scans.pcd", *cloud_from_pcd) == -1)
                    {
                        PCL_ERROR("Couldn't read the pcd file\n");
                        return (-1);
                    }

                    // 将pcd文件中的点云数据复制到init_feats_world
                    for (size_t i = 0; i < cloud_from_pcd->size(); i++) {
                        PointType point;
                        point.x = cloud_from_pcd->points[i].x;
                        point.y = cloud_from_pcd->points[i].y;
                        point.z = cloud_from_pcd->points[i].z;
                        point.intensity = cloud_from_pcd->points[i].intensity;
                        point.normal_x = cloud_from_pcd->points[i].normal_x;
                        point.normal_y = cloud_from_pcd->points[i].normal_y;
                        point.normal_z = cloud_from_pcd->points[i].normal_z;
                        point.curvature = cloud_from_pcd->points[i].curvature;
